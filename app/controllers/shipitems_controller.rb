...
class ShipitemsController < ApplicationController
  ...
  def index
    if params[:search]
      @shipitems = ShipItem.search(params[:search]).order("created_at DESC").paginate(:page => params[:page], :per_page => 15)
    else
      @shipitems = ShipItem.all.order('created_at DESC').paginate(:page => params[:page], :per_page => 15)
    end
  end
